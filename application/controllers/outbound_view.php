...
class Outbound_view extends CI_Controller {
	...
	public function index()
	{
		...
		if (strpos($_POST['data_id'], '-fvt-') !== false) {
			$di=explode('-',$_POST['data_id']);
			$data['id_ves_voyage'] = $di[0];
			if($di[1]!='0'){
				$data['filter'] = $di[2];
				$data['valfill'] = $di[2];
			}else{
		...
		if ($data['id_ves_voyage'] != '')
		{
			...
			$data['special_handling'] 	= $this->container->data_cont_spec_hand();
	...
	public function refresh_index($tab_id, $id_ves_voyage){
		...
		$data['valfill'] 		= $_GET['valfill'];
		
		if(!empty($_GET['valfill'])){
			$data['filter'] = $_GET['valfill'];
		}
	...
	public function print_bay($vescode,$id_ves_voyage,$idbay,$deck_hatch,$nobay,$posisibay)
	{
		...
		$this->load->view('templates/outbound_view/print_partial_bay',$data);
	}
	...
	public function exchange_stowage(){
		...
		if($arrFrom[8] != '20'){
			$check = $this->container->get_outbound_sequence_exchange($id_ves_voyage, $arrTo[1], $arrTo[2],$arrTo[3], $arrFrom[8]);
			if(empty($check)){
				echo json_encode($this->container->exchange_stowage($id_ves_voyage,$from,$to));
			} else {
				echo json_encode(array('F','Slot already taken'));
			}
		} else {
			echo json_encode($this->container->exchange_stowage($id_ves_voyage,$from,$to));
		}
