				...
				foreach($bay_area as $bay){
					...
					if ($bay['BAY']%2!=0){
						...
						//------------ code pembentukan class ------------//
														...
														?>
														<li id_ves_voyage="<?=$id_ves_voyage?>" class_code="<?=$class_code?>" id_vessel="<?=$ID_VESSEL?>" id_bay="<?=$bay['ID_BAY']?>" <?php if ($cell['NO_CONTAINER']!=''){ ?> 
							...
							for($j = 1; $j <= $bay["JML_TIER_ON"]; $j++){
								for($s = 1; $s <= $bay["JML_ROW"]; $s++){
									...
									<li id_ves_voyage="<?=$id_ves_voyage?>" class_code="<?=$class_code?>" id_vessel="<?=$ID_VESSEL?>" id_bay="<?=$bay['ID_BAY']?>" <?php if ($cell['NO_CONTAINER']!=''){ ?> 
									    no_container="<?=$cell['NO_CONTAINER']?>" 
									    point="<?=$cell['POINT']?>" 
									    title="<?=$cell['NO_CONTAINER'].', '.$cell['ID_POD'].', '.$cell['ID_OPERATOR']?>" 
									    cont_size="<?=$cell['CONT_SIZE']?>" 
									    data-pod="<?=$cell['ID_POD']?>" 
									    data-pod-color="<?=$cell['POD_COLOR']?>" 
									    data-opr="<?=$cell['ID_OPERATOR']?>" 
						...
						<ol class="selectable_<?=$tab_id?>">
							...
							for($j = 1; $j <= $bay["JML_TIER_UNDER"]; $j++){
								for($s = 1; $s <= $bay["JML_ROW"]; $s++){
									...
									?>
									<li id_ves_voyage="<?=$id_ves_voyage?>" class_code="<?=$class_code?>" id_vessel="<?=$ID_VESSEL?>" id_bay="<?=$bay['ID_BAY']?>" 
