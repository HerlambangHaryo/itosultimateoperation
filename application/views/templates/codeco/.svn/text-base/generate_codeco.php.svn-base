<script type="text/javascript">
	Ext.onReady(function(){
		Ext.create('Ext.form.Panel', {
			id: "generate_codeco_form_<?=$tab_id?>",
			bodyPadding: 5,
			fieldDefaults: {
				labelAlign: 'left',
				labelWidth: 100
			...
			width: 500,
			url: '<?=controller_?>codeco/generate_codeco',
			items: [{
				xtype: 'hiddenfield',
				name: 'ID_VES_VOYAGE',
				value: '<?=$id_ves_voyage?>'
						...
						form.submit({
							success: function(form, action) {
								...
								if (typeFilenya=='edifact'){
									...
									Ext.getCmp('codeco_list_grid_<?=$tab_id?>').getStore().reload();
								}else{
		...
		}).render('generate_codeco_<?=$tab_id?>');
	});
</script>
<div id="generate_codeco_<?=$tab_id?>"></div>