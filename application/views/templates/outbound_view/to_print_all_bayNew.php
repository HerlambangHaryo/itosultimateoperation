	...
	<div class="boxedBig<?=$paramclass;?>">
		<div id="NameBay<?=$paramclass;?>" style="border-spacing: 0;border-collapse: collapse;" align="center">
			...
			<?php 
				...
				$leftclAbv=$startleft;
				$firsttier=0;
				$resCellAbv=$this->vessel->get_cellPerBayPreLoad($ID_VESSEL,$id_ves_voyage,$bay['BAY'],'ABOVE',$ei);
				foreach($resCellAbv as $rcAbv)
				{
					...
					?>
						...
						.boxConteHatch<?=$paramclass;?>b<?=$bay['BAY'];?>rD<?=$rowB;?>
						{
							font-size:<?=($konstantabox/2);?>pt;font-family: calibri, serif;
							width:<?=$konstantabox;?>px;
							height:<?=$konstantabox;?>px;
						...
						?></div>
						<div class="boxConteHatch<?=$paramclass;?>b<?=$bay['BAY'];?>rD<?=$rowB;?>"><? 
							$resRow=$this->vessel->cek_RowStowageStatus($ID_VESSEL,$rcAbv['ID_BAY'],$rcAbv['ROW_']);
							if($resRow['RES']>0)
							{
								echo $rcAbv['ROW_'];
					...
					><b><? if(($rcAbv['CONT_TYPE']=='RFR') and ($rcAbv['CONT_STATUS']=='FCL')) { echo 'R';} else if (($rcAbv['CONT_TYPE']=='TNK') and ($rcAbv['CONT_STATUS']=='FCL')){ echo 'T';}else if ($rcAbv['CONT_STATUS']=='MTY'){ echo 'e';}else if ($rcAbv['PLUGGING']=='Y'){ echo '.';};?></b></div>
					<?
					if($rcAbv['HAZARD']=='Y'){
							$classy='boxContRbs';
					...
					<div 
					class="<?=$classy;?><?=$paramclass;?>b<?=$bay['BAY'];?>rc<?=$rcAbv['CELL_NUMBER'];?>" style="<? if($rcAbv['CONT_TYPE']=='HQ'){?>text-align:left<?}?>"
					><b><? if ($rcAbv['HAZARD']=='Y'){ echo $rcAbv['IMDG']; }?></b></div>
					<?
					}
			...
			?>
				...
				$leftclAbv=$startleft;
				$firsttier=0;
				$resCellAbv=$this->vessel->get_cellPerBayPreLoad($ID_VESSEL,$id_ves_voyage,$bay['BAY'],'BELOW',$ei);
				foreach($resCellAbv as $rcAbv)
				{
...
<style>
	...
	{
		left:20px;
		top:<?=($maxdivheight+($konstantabox*4)+$top1);?>px;
		position:absolute;
