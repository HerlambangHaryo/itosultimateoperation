...
// print_r($vescode." ");
		...
		$pos = $pss_bay == 'D' ? 'ABOVE' : 'BELOW';
		$id_user = $this->session->userdata('id_user');	

		//======================= Header ===========================//

		$header = $this->vessel->stowage_header_print($vescode,$id_ves_voyage);
		$plan_mch = $this->vessel->get_machine_plan($vescode, $id_ves_voyage, $ei, $idbay);
		...
		$html .= '<br/>
				  ...
				  <tbody>
							...
							for($r=1;$r<=($jumlah_row+1);$r++){
								if($pss_bay=='D')
									...
									if($cell_number<($jumlah_row+1))
									{
										...
										{
											$summary_weight = $this->vessel->summary_weight_out($id_ves_voyage,$no_bay,$row_bay,$pss_bay)->TOTAL;
					...
					$bay_cell = $this->vessel->get_vessel_profile_cellInfo($id_ves_voyage, $ei, $vescode, $id_bay, $no_bay, $pos);
					$index = 0;
							for($t=1;$t<=$height;$t++)
							{
								$html .= '<tr>';
								for($r=1;$r<=($jumlah_row+1);$r++)
								{
									...
									$vesbay_cell = $this->vessel->stowage_print_vescell($id_bay,$cell_number,$pss_bay);
//									$vesbay_cell = $this->vessel->get_vessel_profile_cellInfo($id_ves_voyage, $ei, $ID_VESSEL, $bay['ID_BAY'], $bay['BAY'], 'BELOW');
									foreach ($vesbay_cell as $row_vescell)
									{
										$id_cellx = $row_vescell['ID'];
										$br = $n;
...
//											$ydloc = ($data_cont[17] == 'Y') ? '--' : $ydloc;
											$nocont = $cell['NO_CONTAINER'];
											$sz = $cell['CONT_SIZE'];
											$ty = $cell['CONT_TYPE'];
											$pod = $cell['ID_POD'];
											$pol = $cell['ID_POL'];
											$carrier = $cell['ID_OPERATOR'];
											$gross = $cell['WEIGHT'];
											$ht = $cell['CONT_HEIGHT'];
											$isocode = $cell['ID_ISO_CODE'];
											$st = $cell['CONT_STATUS'];
											$vsbay = $no_bay;
											$ydloc = $cell['YD_LOCATION'];
											$vstatusload = $cell['STATUS'];
											$tl_flag = ($cell['TL_FLAG']=='Y') ? '-TL' : '';
											$commodity = $cell['ID_COMMODITY'];
											$id_class_code = $cell['ID_CLASS_CODE'];
											$esy = ($cell['ITT_FLAG'] == 'Y') ? '-ESY' : '';
//											$pod_color = get_pod_color($pod);
...
//												if (($sz=='40')&&($no_bay<>$vsbay))
												if ($cell['CONT_40_LOCATION'] != '')
												{
													...
													{
														$classor='allocation2';
//														$isi='<br/>'.$line1.'<br/>'.$pol.'&nbsp;&nbsp;&nbsp;'.$pod.'<br/><b>'.$nocont.'</b><br/>'.$carrier.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$gross.'<br/>'.$sz.$ty.'&nbsp;&nbsp;'.$commodity.'&nbsp;&nbsp;&nbsp;'.$ht.
//														'<br/>'.$ydloc;	
													}
													else
													{
