<script type="text/javascript">
	Ext.onReady(function(){
		var ct_store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: '<?=controller_?>baplie_outbound/data_bpe_list',
				reader: {
					type: 'json',
					root: 'data',
					totalProperty: 'total'
				...
				{ 
					...
					items: [{
						...
						handler: function(grid, rowIndex, colIndex) {
							...
							window.open('./edifact/baplie/' + rec.get('FILE_NAME'),'Baplie Donwload');
		...
		ct_grid.render('bpe_list_<?=$tab_id?>');
	});
</script>
<br/>
<div id="bpe_list_<?=$tab_id?>"></div>