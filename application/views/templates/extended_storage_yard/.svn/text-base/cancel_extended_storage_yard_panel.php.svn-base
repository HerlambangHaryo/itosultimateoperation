<script type="text/javascript">
	Ext.onReady(function(){
		...
		var ct_store = Ext.create('Ext.data.Store', {
			fields:['NO_CONTAINER', 'POINT', 'ID_ISO_CODE', 'ID_CLASS_CODE', 'ID_OPERATOR', 'CONT_STATUS', 'OP_STATUS_DESC'],
			autoLoad: false,
			remoteSort: true,
			proxy: {
				type: 'ajax',
				url: '<?=controller_?>cancel_extended_storage_yard/data_cancel_extended_storage_yard',
				reader: {
					type: 'json',
					root: 'data',
					totalProperty: 'total'
		...
		var ct_grid = Ext.create('Ext.grid.Panel', {
			...
			columns: [
				...
				{ text: 'STATUS', dataIndex: 'OP_STATUS_DESC', width: 80}
			],
			viewConfig : {
		...
		Ext.create('Ext.form.Panel', {
			...
			url: '<?=controller_?>cancel_extended_storage_yard/save_cancel_extended_storage_yard',
			items: [{
				items: [{
					xtype: 'hiddenfield',
					name: 'ID_VES_VOYAGE',
						...
						fn: function () {
							...
							if (form.isValid()){
								...
								for (i=0;i<container_data.length;i++){
									...
									if(container_data[i].data.OP_STATUS_DESC == 'Stacking'){
										check_stacking += container_data[i].data.NO_CONTAINER + ', ';
									}
								}
								
								if (container_list.length>0){
									if(check_stacking != ""){
										Ext.MessageBox.confirm('Delete', check_stacking+ 'status stacking'+'. Apakah anda yakin ingin mengcancel esy ?', function(btn){
										   if(btn === 'yes'){
													loadmask.show();
													form.submit({
