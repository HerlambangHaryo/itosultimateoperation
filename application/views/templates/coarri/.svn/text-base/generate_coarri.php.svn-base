<script type="text/javascript">
	Ext.onReady(function(){
		Ext.create('Ext.form.Panel', {
			id: "generate_coarri_form_<?=$tab_id?>",
			bodyPadding: 5,
			fieldDefaults: {
				labelAlign: 'left',
				labelWidth: 100
			...
			width: 500,
			url: '<?=controller_?>coarri/generate_coarri',
			items: [{
				xtype: 'hiddenfield',
				name: 'ID_VES_VOYAGE',
				value: '<?=$id_ves_voyage?>'
						...
						form.submit({
							success: function(form, action) {
								...
								if (typeFilenya=='edifact'){
									...
									Ext.getCmp('coarri_list_grid_<?=$tab_id?>').getStore().reload();
								}else{
		...
		}).render('generate_coarri_<?=$tab_id?>');
	});
</script>
<div id="generate_coarri_<?=$tab_id?>"></div>